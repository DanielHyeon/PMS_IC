# PMS 프로젝트 실행 가이드

## 🚀 빠른 시작

### Windows에서 실행하기

1. **Docker Desktop 설치 및 실행**
   - [Docker Desktop 다운로드](https://www.docker.com/products/docker-desktop/)
   - Docker Desktop을 설치하고 실행합니다
   - 시스템 트레이에서 Docker 아이콘이 보이면 준비 완료

2. **프로젝트 실행**
   ```
   더블클릭: menu.bat
   ```
   또는 간단하게
   ```
   더블클릭: start.bat
   ```

3. **접속**
   - Frontend: <http://localhost:5173>
   - Backend: <http://localhost:8083>
   - LLM Service: <http://localhost:8000>
   - Neo4j Browser: <http://localhost:7474> (ID: neo4j, PW: pmspassword123)
   - PgAdmin: <http://localhost:5050> (ID: admin@pms.com, PW: admin)
   - Redis GUI: <http://localhost:8081>

---

## 📁 제공된 실행 스크립트

### 기본 스크립트

| 파일 | 설명 |
|------|------|
| **menu.bat** | 종합 관리 메뉴 (권장) |
| **start.bat** | 프로젝트 시작 |
| **stop.bat** | 프로젝트 중지 |
| **restart.bat** | 서비스 재시작 |
| **status.bat** | 상태 확인 |
| **logs.bat** | 로그 확인 |

### 사용 예시

```batch
# 종합 메뉴 실행 (모든 기능 포함)
menu.bat

# 개별 명령 실행
start.bat     # 시작
status.bat    # 상태 확인
logs.bat      # 로그 확인
stop.bat      # 중지
```

---

## 🛠️ 고급 사용법

### 명령줄에서 직접 실행

```batch
# 전체 서비스 시작
docker-compose up -d

# 로그 확인
docker-compose logs -f

# 특정 서비스만 로그 확인
docker-compose logs -f backend
docker-compose logs -f frontend

# 서비스 중지
docker-compose down

# 모든 데이터 삭제 (주의!)
docker-compose down -v
```

### 개별 서비스 제어

```batch
# 백엔드만 재시작
docker-compose restart backend

# 프론트엔드만 재시작
docker-compose restart frontend

# 데이터베이스만 재시작
docker-compose restart postgres redis
```

### 이미지 다시 빌드

```batch
# 전체 빌드
docker-compose build

# 백엔드만 빌드
docker-compose build backend

# 프론트엔드만 빌드
docker-compose build frontend

# 빌드 후 시작
docker-compose up -d --build
```

---

## 🔧 문제 해결

### 포트 충돌

다른 프로그램이 포트를 사용 중인 경우:

```batch
# Windows에서 포트 사용 확인
netstat -ano | findstr :5173
netstat -ano | findstr :8080
netstat -ano | findstr :5432

# 프로세스 강제 종료
taskkill /PID [PID번호] /F
```

또는 `docker-compose.yml`에서 포트 변경:
```yaml
ports:
  - "5174:5173"  # 5173 대신 5174 사용
```

### 컨테이너 시작 실패

```batch
# 로그 확인
docker-compose logs backend
docker-compose logs frontend

# 강제 재시작
docker-compose down
docker-compose up -d --force-recreate
```

### 데이터베이스 연결 실패

```batch
# PostgreSQL 상태 확인
docker-compose ps postgres

# 수동 연결 테스트
docker-compose exec postgres pg_isready -U pms_user

# 환경 변수 확인
docker-compose exec backend env | findstr SPRING_DATASOURCE
```

### Hot Reload 작동 안 함

- 프론트엔드: `docker-compose.override.yml` 확인
- 백엔드: Spring DevTools 활성화 확인

---

## 📊 서비스 구성

```
┌─────────────────────────────────────────────────────┐
│                  Docker Compose                      │
├─────────────────────────────────────────────────────┤
│                                                      │
│  ┌──────────────┐  ┌──────────────┐  ┌───────────┐ │
│  │   Frontend   │  │   Backend    │  │ PostgreSQL│ │
│  │  React+Vite  │  │ Spring Boot  │  │  (DB)     │ │
│  │  :5173       │  │  :8080       │  │  :5432    │ │
│  └──────────────┘  └──────────────┘  └───────────┘ │
│                                                      │
│  ┌──────────────┐  ┌──────────────┐  ┌───────────┐ │
│  │    Redis     │  │  AI Service  │  │  PgAdmin  │ │
│  │  (Cache)     │  │   (Mock)     │  │   (GUI)   │ │
│  │  :6379       │  │  :8000       │  │  :5050    │ │
│  └──────────────┘  └──────────────┘  └───────────┘ │
│                                                      │
└─────────────────────────────────────────────────────┘
```

### 포트 매핑

| 서비스 | 포트 | URL |
|--------|------|-----|
| Frontend | 5173 | http://localhost:5173 |
| Backend | 8080 | http://localhost:8080 |
| PostgreSQL | 5432 | localhost:5432 |
| Redis | 6379 | localhost:6379 |
| PgAdmin | 5050 | http://localhost:5050 |
| Redis Commander | 8082 | http://localhost:8082 |
| AI Service Mock | 8000 | http://localhost:8000 |

---

## 🔐 기본 계정 정보

### PgAdmin
- URL: http://localhost:5050
- Email: admin@pms.com
- Password: admin

### PostgreSQL
- Host: localhost
- Port: 5432
- Database: pms_db
- User: pms_user
- Password: pms_password

### Redis
- Host: localhost
- Port: 6379
- Password: (없음)

---

## 📝 개발 워크플로우

### 1. 프로젝트 시작
```batch
menu.bat  # 또는 start.bat
```

### 2. 코드 수정
- 프론트엔드: `PMS_IC_FrontEnd_v1.2/src` 디렉토리
- 백엔드: `PMS_IC_BackEnd_v1.2/src` 디렉토리
- Hot Reload가 활성화되어 있어 저장하면 자동 반영

### 3. 로그 확인
```batch
logs.bat  # 또는 docker-compose logs -f
```

### 4. 데이터베이스 작업
```batch
# PostgreSQL 접속
docker-compose exec postgres psql -U pms_user -d pms_db

# 테이블 확인
docker-compose exec postgres psql -U pms_user -d pms_db -c "\dt auth.*"
```

### 5. Redis 작업
```batch
# Redis CLI 접속
docker-compose exec redis redis-cli

# 키 조회
docker-compose exec redis redis-cli KEYS "*"
```

---

## 🎯 체크리스트

개발 시작 전 확인사항:

- [ ] Docker Desktop 설치 및 실행
- [ ] `.env` 파일 생성 (start.bat 실행 시 자동 생성)
- [ ] 필요한 포트 사용 가능 (5173, 8080, 5432, 6379 등)
- [ ] `menu.bat` 또는 `start.bat` 실행
- [ ] 모든 서비스 정상 작동 확인 (`status.bat`)

---

## 💡 유용한 팁

### 성능 모니터링
```batch
# 리소스 사용량 확인
docker stats
```

### 데이터 백업
```batch
# 데이터베이스 백업
docker-compose exec postgres pg_dump -U pms_user pms_db > backup.sql

# 복원
docker-compose exec -T postgres psql -U pms_user -d pms_db < backup.sql
```

### 로그 레벨 조정
`docker-compose.override.yml` 파일에 추가:
```yaml
services:
  backend:
    environment:
      LOGGING_LEVEL_ROOT: DEBUG
```

---

## 📚 추가 문서

- [README_DOCKER.md](README_DOCKER.md) - 상세 Docker 가이드
- [Makefile](Makefile) - Linux/Mac용 명령어
- [Backend README](PMS_IC_BackEnd_v1.2/README.md)
- [Frontend README](PMS_IC_FrontEnd_v1.2/README.md)

---

## 🆘 도움말

문제가 발생하면:

1. `status.bat`으로 상태 확인
2. `logs.bat`으로 로그 확인
3. Docker Desktop이 실행 중인지 확인
4. 필요한 포트가 사용 가능한지 확인
5. `docker-compose down` 후 다시 시작

**긴급 복구:**
```batch
docker-compose down -v
docker-compose up -d --force-recreate
```

---

**행운을 빕니다! 🚀**
