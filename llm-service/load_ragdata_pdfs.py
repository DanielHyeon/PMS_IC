"""
ragdata/ 폴더의 PDF 파일들을 읽어서 벡터 DB에 추가
...
def load_pdf_files(ragdata_dir: str = "../ragdata"):
    """
    ragdata 디렉토리의 PDF 파일들을 로드하여 벡터 DB에 추가
    ...
    logger.info("=" * 80)
    logger.info("ragdata PDF 파일 벡터 DB 추가")
    logger.info("=" * 80)
        ...
        for i, pdf_file in enumerate(pdf_files, 1):
            try:
                ...
                if rag_service.add_document(document):
                    ...
                    logger.info(f"   ✅ 완료: {chunk_count}개 청크 생성")
                else:
                    logger.error(f"   ❌ 실패")
...
- 파서: {stats.get('parser')}
            ...
            if results:
                for i, result in enumerate(results, 1):
                    ...
                    if source == 'mineru_model':
                        logger.info(f"    파싱: 🤖 MinerU 모델")
                    else:
                        logger.info(f"    파싱: 📐 휴리스틱")
        ...
        logger.info("\n" + "=" * 80)
        logger.info("✅ ragdata PDF 파일 벡터 DB 추가 완료!")
        logger.info("=" * 80)
